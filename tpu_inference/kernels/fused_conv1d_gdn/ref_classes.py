...
@jax.tree_util.register_dataclass
@dataclasses.dataclass(frozen=True)
class OptionalSmemWithDefaultFn:
    data: Any | None = None
    default_fn: Any = dataclasses.field(default=None,
                                        metadata=dict(static=True))

    def __post_init__(self):
        assert (self.data is not None) ^ (self.default_fn is not None), (
            "Must provide exactly one of data or default_fn")

    def __getitem__(self, indices) -> jax.Array | int:
        if self.data is None:
            return self.default_fn(indices)
...
class MetadataRef:
    num_tiles: Any
    start_st_idx: OptionalSmemWithDefaultFn
    st_idx_to_t_idx: OptionalSmemWithDefaultFn
    st_idx_to_s_idx: OptionalSmemWithDefaultFn
    st_idx_to_b_idx: OptionalSmemWithDefaultFn
    st_idx_to_b_size: OptionalSmemWithDefaultFn
    ...
    @staticmethod
    def _wrap_optional_smem(x: Any) -> OptionalSmemWithDefaultFn:
        match x:
            case Callable():
                return OptionalSmemWithDefaultFn(default_fn=x)
            case jax.Array():
                return OptionalSmemWithDefaultFn(data=x)
            case _:
                raise TypeError(
    ...
    def create(
        cls,
        num_tiles: jax.Array,
        start_st_idx: Callable[..., int] | jax.Array,
        st_idx_to_t_idx: Callable[..., int] | jax.Array,
        st_idx_to_s_idx: Callable[..., int] | jax.Array,
        st_idx_to_b_idx: Callable[..., int] | jax.Array,
        st_idx_to_b_size: Callable[..., int] | jax.Array,
