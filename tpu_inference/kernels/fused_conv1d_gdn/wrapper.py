...
def load_activation_as_compact(
    ...
    q_compact, k_compact, v_compact = strided_ldst.load_as_qkv_compact(
        qkv_vmem_ref, cfgs)
...
def load_activation_as_large(
    ...
    for s_idx in range(cfgs.seq_tile_size):
        iota = jax.lax.broadcasted_iota(jnp.int32, qkv_vreg[s_idx].shape, 0)
        masked_qkv = jnp.where(iota < real_size[s_idx], qkv_vreg[s_idx], 0)
        qkv_vmem_ref[s_idx] = masked_qkv

    q_large, k_large, v_large = strided_ldst.load_as_qkv_large(
        qkv_vmem_ref, cfgs)

    b_large_list = []
    a_large_list = []
    for s_idx in range(cfgs.seq_tile_size):
        b_s = strided_ldst.load_compact_to_large(b_vmem_ref.at[s_idx])
        a_s = strided_ldst.load_compact_to_large(a_vmem_ref.at[s_idx])
        iota = jax.lax.broadcasted_iota(jnp.int32, b_s.shape, 0)
        b_s = jnp.where(iota < real_size[s_idx], b_s, 0)
        a_s = jnp.where(iota < real_size[s_idx], a_s, 0)
        b_large_list.append(b_s)
        a_large_list.append(a_s)
...
def inner_kernel(
    ...
    # Prepare states.
    real_size, prev_conv_state, prev_recurrent_state = load_and_mask_states(
    ...
    qkv_in_compact = jnp.concat([prev_conv_state, qkv_in_compact], axis=1)
