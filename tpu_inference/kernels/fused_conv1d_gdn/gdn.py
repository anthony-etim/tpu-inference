...
def invert_triangular_matrix(t: jax.Array, block_size=16) -> jax.Array:
    ...
    # NOTE: if chunk_size=1, compiler will optimize away the entire function.
    out_dtype = t.dtype
    chunk = t.shape[-1]
...
def chunked_gdn(
    ...
    a_log = gdn_weights_ref.a_log[...].reshape(1, 1, -1)
    a_log = a_log.astype(cfgs.dtypes.compute)
    dt_bias = gdn_weights_ref.dt_bias[...].reshape(1, 1, -1)
    ...
    gating_log = jnp.expand_dims(gating_log, 1)
    beta = jnp.expand_dims(beta, 1)
...
def recurrent_gdn(
    ...
    for s_idx in range(cfgs.seq_tile_size):
        ...
        new_recurrent_state_list.append(state)

    out = jnp.stack(out_list, axis=0)
    new_recurrent_state = jnp.stack(new_recurrent_state_list, axis=0)
